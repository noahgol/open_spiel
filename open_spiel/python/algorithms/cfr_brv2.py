"""Python implementation of the CFR-BR V2 algorithm."""
...
class CFRBRV2Solver(_CFRSolverBase):
  ...
  def __init__(self, game, linear_averaging=False, regret_matching_plus=False):
    ...
    super(CFRBRV2Solver, self).__init__(
        game,
  ...
  def evaluate_and_update_policy(self):
    ...
    for player in range(self._num_players):
      ...
      policies = [self._get_infostate_policy for p in range(self._num_players)]
      policies[player] = (
          lambda infostate_str: 
          {self._best_responses[player]["best_response_action"][infostate_str]: 1})
      #policies = [
